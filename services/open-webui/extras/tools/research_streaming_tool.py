...
class Tools:
    ...
    def _track_research_progress_old(self, session_id: str, query: str) -> str:
        ...
        try:
            ...
            # Timeout
            progress_log.append(
                f"⏱️ **Research timed out** after {self.valves.timeout} seconds"
            )
            return "\n\n".join(progress_log)
    ...
    def _get_final_results(self, session_id: str, query: str) -> str:
        ...
        try:
            result_response = requests.get(
                f"{self.valves.researcher_url}/research/{session_id}/result", timeout=30
            )
            ...
            if result_response.status_code == 200:
                ...
                result_parts = []
                result_parts.append(
                    f"# {result.get('title', f'Research Results: {query}')}"
                )
                ...
                if sources:
                    ...
                    if len(sources) > 10:
                        result_parts.append(
                            f"*Plus {len(sources) - 10} more sources...*"
                        )
                ...
                if metadata:
                    ...
                    stats.append(
                        f"- **Search API:** {metadata.get('search_api', 'N/A')}"
                    )
                    stats.append(
                        f"- **Research loops:** {metadata.get('max_loops', 'N/A')}"
                    )
                    stats.append(
                        f"- **Sources analyzed:** {metadata.get('sources_analyzed', 'N/A')}"
                    )
                    stats.append(
                        f"- **Processing time:** {metadata.get('processing_time_seconds', 'N/A')}s"
                    )
                    result_parts.append("\n".join(stats))
