...
- **Each service is now a folder** (`services/<name>/`) containing `service.yml` (manifest — env vars, source variants with declarative `effects:` maps, image refs, dependencies, optional Python `hook:`) and `compose.yml` (Compose fragment). 24 manifests total — 21 container-backed + 3 virtual (cloud-providers, tts-provider, globals). Schema-validated against `bootstrapper/schemas/service.schema.json`.
...
- **Six per-service hooks** at `bootstrapper/services/hooks/`: `comfyui`, `weaviate`, `openclaw`, `cloud_providers` (multi-input — aggregates three `CLOUD_*_SOURCE` toggles), `speaches` (multi-input — activated by either STT_PROVIDER_SOURCE or TTS_PROVIDER_SOURCE), `chatterbox`.
- **`bootstrapper/tests/`** — 140+ tests: loader, cross-manifest validator, env assembler, validate_fragments CLI, hooks, fragment-equivalence (byte-equiv vs golden baseline), source-permutation matrix, env-example consistency (manifest ↔ .env.example parity), backfill interplay (manifest change → backfill → user .env propagation).
