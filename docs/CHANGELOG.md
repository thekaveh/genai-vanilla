...
- **`volumes/api/kong-dynamic.yml` is now a pure runtime artifact** (`.gitignore`d, regenerated on every `./start.sh`). Direct `docker compose up` from a clean checkout is unsupported — `services/kong/README.md` updated; `docs/scripts/check-kong-routes.py` was rewritten to invoke the kong generator against `.env.example` in a tmp dir and validate that, instead of reading the user's runtime file.
