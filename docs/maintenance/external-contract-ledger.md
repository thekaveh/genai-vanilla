...
| MinIO client service-account commands | `minio/mc:RELEASE.2025-04-16T18-13-26Z` | MinIO current docs center `mc admin accesskey`; legacy `mc admin user svcacct` is documented as replacement-bound | `services/minio/init/scripts/init-minio.sh` still uses `mc admin user svcacct` create/edit/list | Deferred pending live MinIO init validation or deliberate migration to `mc admin accesskey` |
| n8n community package install endpoint | n8n image configured by service manifest | Static REST flow trace of `/rest/community-packages` install path | Init now exits nonzero if any required community-node installation fails | Guarded by `bootstrapper/tests/test_shell_script_contracts.py` |
