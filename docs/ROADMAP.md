...
- 5 query modes: naive, local, global, hybrid, mix; `LightRAG_SOURCE` variants: `container`, `disabled`
...
- Kong route at `/lightrag/` (port 63042); `LIGHTRAG_API_KEY` auto-generated by bootstrapper
...
- OpenAI-compatible `/embeddings` + dedicated `/rerank` routes; `TEI_RERANKER_SOURCE` variants: `container-cpu`, `container-gpu`, `disabled`
- Default model: `BAAI/bge-reranker-v2-m3`; LightRAG wires `RERANK_BINDING_HOST` from `TEI_RERANKER_ENDPOINT` when both services are enabled
- Kong route at `/tei-reranker/` (port 63043); model weights cached in a named volume
